package me.coderfrish.traium.i18n;
...
import me.coderfrish.traium.config.TraiumConfig;
...
/*
 * authored by: Frish2021 <1573880184@qq.com>
 * modified by: Bacteriawa <A3167717663@hotmail.com>
 * modified by: Helvetica Volubi <suisuroru@blue-millennium.fun>
 * modified by: Lumine1909 <133463833+Lumine1909@users.noreply.github.com>
 * Some of diff form Leaves
 */
public class TraiumI18n {
    private static final Logger logger = LogUtils.getClassLogger();
    private static final String VERSION = DetectedVersion.BUILT_IN.name();
    private static final String BASE_PATH = "cache/traium/" + VERSION + "/";
    private static final String defaultTraiumLangPath = "/assets/traium/minecraft/lang/en_us.json";
    ...
    private static String traiumLangPath;

    public static void init() {
        String language = TraiumConfig.language;
        if (Objects.equals(language, "en_us")) {
            return;
        }

        langPath = BASE_PATH + "lang/" + language + ".json";
        langJsonPath = "minecraft/lang/" + language + ".json";
        traiumLangPath = "/assets/traium/minecraft/lang/" + language + ".json";
        assetsPath = BASE_PATH + "assets.json";
        versionPath = BASE_PATH + VERSION + ".json";
        manifestPath = BASE_PATH + "manifest.json";

        logger.info("Starting load language: {}", language);

        if (preloadTask == null) {
            preloadTask = CompletableFuture.runAsync(() -> preLoadI18n(2));
        }

        preloadTask.thenAcceptAsync(v -> loadI18n(language, 2));
    }

    public static void preInit() {
        if (Objects.equals(TraiumConfig.language, "en_us")) {
            return;
        }
    ...
    private static void preLoadI18n(int retryTime) {
        try {
            ...
            logger.warn("Unsupported language: {}", TraiumConfig.language);
        } catch (Exception e) {
    ...
    private static void loadTraiumI18n(BiConsumer<String, String> bi) {
        if (Language.class.getResource(traiumLangPath) != null) {
            Language.parseTranslations(bi, traiumLangPath);
        } else {
            loadTraiumI18nDefault(bi);
        }
    }

    public static void loadTraiumI18nDefault(BiConsumer<String, String> bi) {
        if (Language.class.getResource(defaultTraiumLangPath) != null) {
            Language.parseTranslations(bi, defaultTraiumLangPath);
        }
    }
